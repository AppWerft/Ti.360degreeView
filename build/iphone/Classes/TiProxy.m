...
//Rare exceptions to indicate a bug in the _ti360view code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_ti360view = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_ti360view]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.ti360view.module"
